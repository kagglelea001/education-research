...
To assess whether the estimated spillover effects are specific to the Top 100 threshold rather than artifacts of the RDD specification, we conduct three supplementary analyses.

First, we implement placebo tests at false thresholds—the 50th, 150th, and 200th rank positions—replicating the identical RDD procedure at each alternative cutoff. If the identified effects were artifacts of the estimation method rather than genuine threshold effects, significant "treatment" estimates would emerge at these arbitrary cutoffs. Across 12 placebo specifications (3 false thresholds × 4 bandwidths), none produces statistically significant estimates at conventional levels (all p-values > 0.05). This null finding supports the interpretation that the Top 100 threshold carries unique causal significance, consistent with the literature on the salience of round-number rankings in higher education (Hazelkorn, 2015).

Second, we estimate a "donut" RDD specification that excludes observations with score gaps exactly equal to zero, addressing potential concerns about measurement precision at the threshold boundary. The results remain consistent in sign with our baseline estimates, confirming that the findings are not driven by a small number of observations precisely at the cutoff.

Third, we implement a quadratic polynomial specification to allow for nonlinear curvature in the relationship between the running variable and outcomes. The results are robust across bandwidths (h = 15: β = +1.09, p = 0.005), confirming that the linear specification does not impose functional form assumptions that drive our conclusions.

Fourth, we incorporate external GDP per capita as a control variable, using the residualized approach to partial out the influence of national economic development from the RDD estimates. Controlling for GDP does not alter the substantive pattern of results, indicating that the spillover effects are not merely reflecting cross-national differences in economic development levels.

### Heterogeneity Analysis

To investigate whether the spillover effects vary systematically across institutional and national contexts, we conduct subgroup analyses along two dimensions.

By national development level, the positive spillover effects on university scores are primarily driven by developed countries (β = +5.72, p < 0.001 at h = 5; β = +3.50, p = 0.006 at h = 8), where the institutional infrastructure for translating reputational gains into measurable improvement is more robust. By contrast, developing countries in our sample exhibit insufficient within-bandwidth observations for reliable subgroup estimation, a limitation discussed further in the following section.

By university tier, a distinctive gradient emerges. Lower-tier institutions (ranked 401 and below) exhibit the strongest positive spillovers (β = +2.16, p = 0.004 at h = 8), suggesting that the signaling value of a national Top 100 university is most potent for institutions that benefit most from reputational enhancement. Near-elite universities (ranked 100–200), by contrast, exhibit negative estimates (β = −3.57, p = 0.007 at h = 8), consistent with intensified resource competition: these institutions, being closest to the elite threshold, may be most affected by the concentration of research resources and faculty talent at the newly elevated flagship. Mid-tier institutions (ranked 201–400) show no systematic spillover effects, consistent with an intermediate position that neither benefits substantially from the brand signal nor loses substantially from resource concentration. This gradient pattern provides suggestive evidence for the resource concentration mechanism: the negative effects are strongest precisely where competition for overlapping resources—elite faculty, research funding, high-performing doctoral students—is most intense.
...
### Cross-Ranking Validation

A natural concern with any analysis reliant on a single ranking system is whether the findings are specific to that system's methodology. To assess this, we conduct a cross-validation exercise using a dataset of 57 universities that appear in both the QS 2026 and THE 2026 rankings, enabling direct comparison of institutional scores across the two systems. The Spearman rank correlation between QS and THE scores is ρ = 0.815 (p < 0.001), indicating strong but imperfect agreement. The Top 100 overlap between the two systems is 92 percent (48 out of 52 QS Top 100 universities also appear in THE's Top 100). At the sub-indicator level, QS academic reputation correlates with THE teaching score at r = 0.885 (p < 0.001), employer reputation with industry income at r = 0.739 (p < 0.001), and international student metrics across systems at r = 0.748 (p < 0.001). These strong cross-system correlations provide reassurance that QS sub-scores capture genuine institutional characteristics rather than being mere artifacts of QS's particular methodology. They also suggest that the main pattern of findings—positive market-facing spillovers, negative research concentration effects—would likely replicate in other ranking systems that satisfy the McCrary validity condition, though this conjecture awaits direct empirical confirmation when suitable multi-system panel data become available.
